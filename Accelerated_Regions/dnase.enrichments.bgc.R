#exactly the same as dnase enrichments but for bgc
...
hg.acc <- import.bed('../generalstuff/July Stuff/Accelerated_Regions.July2017/GC_bias/hg19.GCbias.bed')
mm.acc <- import.bed('../generalstuff/July Stuff/Accelerated_Regions.July2017/GC_bias/mm10.GCbias.bed')
...
accel_beds <- Sys.glob('~/Documents/Projects/Accelerated_Regions/generalstuff/July Stuff/Accelerated_Regions.July2017/GC_bias/*.bed') #GC_bias/noGCbias/*bed')
for(i in accel_beds){
...
hg.hgav <- apply(hg.hAd[,-c(1:5)],2,w.mean)
hg.mmav <- apply(hg.mAd[,-c(1:5)],2,w.mean)
hg.noav <- apply(hg.noAd[,-c(1:5)],2,w.mean)
...
mm.hgav <- apply(mm.hAd[,-c(1:5)],2,w.mean)
mm.mmav <- apply(mm.mAd[,-c(1:5)],2,w.mean)
mm.noav <- apply(mm.noAd[,-c(1:5)],2,w.mean)
