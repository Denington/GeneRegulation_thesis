...
rad21.sites <- read.table('~/Documents/Projects/Thymocyte_HiC/Differential_chip/WTvsCTCFko.RAD21.bed',sep = '\t',col.names = 
...
genes= resize(conds.gr.list$ctcf_ko,500,fix='center')
...
ctcfkoU <- ScoreMatrixBin(ctcfko.Ins,resize(up.boundaries,250000,fix='center'),bin.num = 25)

wtD <- ScoreMatrixBin(wt1.Ins,resize(down.boundaries,250000,fix='center'),bin.num = 25)
ctcfkoD <- ScoreMatrixBin(ctcfko.Ins,resize(down.boundaries,250000,fix='center'),bin.num = 25)

wtS <- ScoreMatrixBin(wt1.Ins,resize(stab.boundaries,250000,fix='center'),bin.num = 25)
ctcfkoS <- ScoreMatrixBin(ctcfko.Ins,resize(stab.boundaries,250000,fix='center'),bin.num = 25)
...
plotMeta(ScoreMatrixList(list(up.diff,down.diff,stab.diff)),profile.names = c('Up.diff','Down.diff','Stab.diff'),dispersion = 'se', 
         ylab='CD69-DP WT - CTCFKO Insulation',xlab = 'Distance from boundary',xcoords = seq(-125,125,length.out = 25))
abline(v=c(0),lty=2)

#should do the other way round- at boundaries (or I have done already, but not looked at change in rad21?) 
#not what looking for here, but upregulated rad21 tend to be in internal tads? (or at least at subtad boundaries) - it is whered i get that?
#also it's interesting to compare to the dev changes. For dev, change is fairly consistent across all bins- lines are near flat
#whilst here I still see a funnel shape. Which I think means that insulation is changing consistently around the boundary through dev
#and weakened more directly at boundaries for ctcfko?
...
A3_4_9 <- plot_boundary_vs_rad21(ctcf_wt_difbounds)
C3_4_10 <- plot_boundary_vs_rad21(dev_difbounds)
D3_4_10 <- plot_boundary_vs_rad21(dko_wt_difbounds)

fig_3_4_10 <- plot_grid(C3_4_10,D3_4_10,nrow = 1,labels = c('C','D'))

save_plot('~/Desktop/Fig_3_4_9A.png',A3_4_9,base_asp = 1)
save_plot('~/Desktop/Fig_3_4_10CD.png',fig_3_4_10)
...
ggplot(change.exp.rad21,aes(x=status,y=best.logFC. , fill=status)) + geom_boxplot(outlier.alpha = 0.01) + theme_classic() + 
  ...
  ylab('log Fold Change in RAD21 signal') + xlab('Gene Status ΔCTCF')
...
#ctcf_bed <- ctcf_bed[width(ctcf_bed) < 600]
...
all.genes.wt.rad21 <- ScoreMatrixBin(rad21_file,resize(conds.gr.list$ctcf_ko,2000,fix='center'),bin.num = 101,type="bw",strand.aware = T)
all.genes.ctcfko.rad21 <- ScoreMatrixBin(ctcfko.rad21_file,resize(conds.gr.list$ctcf_ko,2000,fix='center'),bin.num = 101,type="bw",strand.aware = T)

ctcfsites.wt.rad21 <- ScoreMatrixBin(rad21_file,resize(ctcf_bed,2000,fix='center'),bin.num = 101,type="bw",strand.aware = T)
ctcfsites.ctcfko.rad21 <- ScoreMatrixBin(ctcfko.rad21_file,resize(ctcf_bed,2000,fix='center'),bin.num = 101,type="bw",strand.aware = T)


#at active genes
multiHeatMatrix(ScoreMatrixList(list(all.genes.wt.rad21,all.genes.ctcfko.rad21)),winsorize = c(1,99),
                col=blues9,matrix.main = c('WT','ΔCTCF'),order = T,common.scale = T,
                xlab = 'Distance from promoter (bp)',xcoords = c(-1000,1000))
plotMeta(ScoreMatrixList(list(all.genes.wt.rad21,all.genes.ctcfko.rad21)),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],
         xcoords = c(-1000,1000),xlab = 'Distance from promoter (bp)',ylab='RAD21 signal')
abline(v=0, lty=2)
...
multiHeatMatrix(ScoreMatrixList(list(ctcfsites.wt.rad21,ctcfsites.ctcfko.rad21)),winsorize = c(1,99),
                col=blues9,matrix.main = c('WT','ΔCTCF'),common.scale = T,order = T,
                xcoords = c(-1000,1000),xlab='Distance from CTCF site (bp)')
plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21,ctcfsites.ctcfko.rad21)),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],
         xcoords = c(-1000,1000),xlab='Distance from CTCF site (bp)', ylab = 'average RAD21 signal')

abline(v=0, lty=2)

plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21[1:1000],ctcfsites.ctcfko.rad21[1:1000])),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],xcoords = c(-1000,1000),
         xlab='Distance from CTCF peak (bp)', ylab='RAD21 signal')
abline(v=0, lty=2)

plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21[1000:5000],ctcfsites.ctcfko.rad21[1000:5000])),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],xcoords = c(-1000,1000),
         xlab='Distance from CTCF peak (bp)', ylab='RAD21 signal')
abline(v=0, lty=2)

plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21[5000:10000],ctcfsites.ctcfko.rad21[5000:10000])),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],xcoords = c(-1000,1000),
         xlab='Distance from CTCF peak (bp)', ylab='RAD21 signal')
abline(v=0, lty=2)

plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21[10000:15000],ctcfsites.ctcfko.rad21[10000:15000])),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],xcoords = c(-1000,1000),
         xlab='Distance from CTCF peak (bp)', ylab='RAD21 signal')
abline(v=0, lty=2)

plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21[15000:20000],ctcfsites.ctcfko.rad21[15000:20000])),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],xcoords = c(-1000,1000),
         xlab='Distance from CTCF peak (bp)', ylab='RAD21 signal')
abline(v=0, lty=2)

plotMeta(ScoreMatrixList(list(ctcfsites.wt.rad21[20000:25000],ctcfsites.ctcfko.rad21[20000:25000])),winsorize = c(1,99),profile.names = c('WT','ΔCTCF'),
         dispersion='se',line.col = rainbow(20)[c(1,13)],dispersion.col = rainbow(20,alpha=0.25)[c(1,13)],xcoords = c(-1000,1000),
         xlab='Distance from CTCF peak (bp)', ylab='RAD21 signal')
abline(v=0, lty=2)

#stronger binding site- less difference
##############
##############

#####now looking at how other cond gene changes link to ctcfko rad21  
...
getlfs <- function(genes,nam){
  ...
  ggplot(change.exp.rad21,aes(x=status,y=best.logFC. , fill=status)) + geom_boxplot(outlier.alpha = 0.01) + theme_classic() + 
    scale_fill_manual(values=c('#0073C2FF','#868686FF','#EFC000FF')) + ggpubr::stat_compare_means(comparisons = comparisons) +
    ylab('log Fold Change in RAD21 signal post-ΔCTCF') + xlab(paste0('Gene Status ',nam))
}

getlfs(resize(conds.gr.list$dev,500,fix='center'),'through Development')
getlfs(resize(conds.gr.list$dko,500,fix='center'),'post-ΔCTCF/ΔRAD21')
